...
Q2: Does New York differ from California in its clearance rates for all types of crime over time? (visualization)
...
ggplot(crime_clean,aes(x=year,y=violent_avg,color=state))+
  ...
  theme_minimal()+
  labs(x="Year",y="Average clearance rate for violent crime (%)", title="Violent crime clearance rates over time\nTop 5 most populated states")
...
ggplot(crime_clean,aes(x=year,y=property_avg,color=state))+
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))+
  theme_minimal()+
  labs(x="Year",y="Average clearance rate for property crime (%)", title="Property crime clearance rates over time\nTop 5 most populated states")
