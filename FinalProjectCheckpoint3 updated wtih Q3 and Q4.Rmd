...
ggplot(crime_clean,aes(x=year,y=violent_avg,color=state))+
  ...
  labs(x="state",y="clearance rate for violent crime", title="clearance rate for violent crime by the top 5 most populated states")

ggplot(crime_clean,aes(x=year,y=property_avg,color=state))+
  geom_line(size=1.2)+
  theme(axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1))+
  labs(x="state",y="clearance rate for property crime", title="clearance rate for property crime by the top 5 most populated states")
...
crime_clean$high_clearance <- ifelse(
  crime_clean$clearance_rate_total_crime >= 65, 1, 0
)

# Build dataset for Q4
crime_q4 <- crime_clean %>%
  select(
    high_clearance,
    new_assault, new_robbery, new_burglary,
    new_theft, new_motor_vehicle_theft,
