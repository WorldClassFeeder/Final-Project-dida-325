...
```{r}
# Property crime (2020)
ggplot(crime_top5pop2020, aes(x = state, y = clearance_rate_property_crime, fill = state)) +
  ...
  labs(
    x = "State",
    y = "Property crime clearance rate (%)",
    title = "Property crime clearance rates (2020)\nTop 5 most populated states"
  )

# Violent crime (2020)
ggplot(crime_top5pop2020, aes(x = state, y = clearance_rate_violent_crime, fill = state)) +
  geom_bar(stat = "identity") +
  scale_y_continuous(limits = c(0, 100)) +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1, vjust = 1)) +
  labs(
    x = "State",
    y = "Violent crime clearance rate (%)",
    title = "Violent crime clearance rates (2020)\nTop 5 most populated states"
  )
```
The barplots show how often police cleared violent and property crimes in 2020 for the five states with the largest populations. Pennsylvania clears the highest share of both crime types - Texas clears the lowest. Florida besides New York sit between those two extremes, with rates that differ only slightly from one another.

Because every state filed a full set of numbers for 2020, the picture is not warped by gaps, unlike the long term trends asked about in Question 1. Florida's rate looks steady, since no data points are absent. Across the board the state-to-state gaps are large - yet Texas still ranks last for clearing both violent and property offences.
